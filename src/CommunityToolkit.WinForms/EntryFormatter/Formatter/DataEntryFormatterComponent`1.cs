...
public abstract partial class DataEntryFormatterComponent<T> :
    BindableComponent, IExtenderProvider, IDataEntryFormatterComponent
{
    /// <summary>
    ...
    private readonly Dictionary<Control, IDataEntryFormatter<T>> _propertyStorage = [];
    ...
    /// <returns>
    /// The <see cref="IDataEntryFormatter{T}"/> associated with the control, or <see langword="null"/> if not set.
    /// </returns>
    [Category("Data Entry Extension")]
    [DisplayName("-> FormatterSettings")]
    [RefreshProperties(RefreshProperties.All)]
    public IDataEntryFormatter<T>? GetFormatterSettings(Control textBox)
        => _propertyStorage.TryGetValue(textBox, out IDataEntryFormatter<T>? value)
            ? value
            : null;
    ...
    /// <param name="value">The <see cref="IDataEntryFormatter{T}"/> to associate with the control.</param>
    public void SetFormatterSettings(Control textBox, IDataEntryFormatter<T> value)
    ...
    /// <inheritdoc/>
    object? IDataEntryFormatterComponent.GetValue(TextBox textBox)
        => GetValue(textBox);

    /// <inheritdoc/>
    void IDataEntryFormatterComponent.SetValue(TextBox textBox, object? value)
    ...
    public async Task<bool> TryConvertToValueAsync(TextBox textBox, string? stringValue)
    {
        ...
        try
        {
            IDataEntryFormatter<T>? temp = GetFormatterSettings(textBox);
    ...
    /// </summary>
    /// <returns>An instance of <see cref="IDataEntryFormatter{T}"/>.</returns>
